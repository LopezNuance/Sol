...
**v0.3.0 — FROZEN (2026-09-11), Revision 3**
...
SOL-QRY v0.3.0 passed the §44 freeze gate 19/19 on 2026-09-11 and is frozen for implementation. The gate run is evidenced by `freeze_report_v030.json` (status FROZEN, report digest `sha256:1ea510ee17a5d8b48677678c106f55a5a768b0b76f822bb771187c49c2884b83`); every cited artifact is a committed repository file and every digest recomputes from it via the gate harness (`qry_reference_impl/gate_check.py`). The historical `freeze_report.json` (v0.1 reconstruction, 18/19, NOT_FROZEN) is retained as evidence and is not amended. Findings during the frozen implementation period are triaged per §44 (errata, corpus entry, unfreeze candidate); unfreeze candidates are batched into the next semantic revision.
